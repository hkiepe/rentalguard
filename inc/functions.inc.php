...
function check_user() {
	...
	if(!isset($_SESSION['userid']) && isset($_COOKIE['identifier']) && isset($_COOKIE['securitytoken'])) {
		...
		if(sha1($securitytoken) !== $securitytoken_row['securitytoken']) {
			...
			setcookie("identifier",$identifier,time()+(3600*15)); //15 Stunden Gültigkeit
			setcookie("securitytoken",$neuer_securitytoken,time()+(3600*15)); //15 Stunden Gültigkeit
	...
	if(!isset($_SESSION['userid'])) {
		die('Najpierw proszę <a href="login.php">się zaloguj</a>');
	}
